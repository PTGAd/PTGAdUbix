...
- (PTGSplashAd *)splashAd {
    if (!_splashAd) {
        ...
        _splashAd = [[PTGSplashAd alloc] initWithPlacementId:@"46453_1"];
...
- (PTGNativeExpressAdManager *)manager {
    if (!_manager) {
      /// draw 信息流 adSize传入屏幕的size
        _manager = [[PTGNativeExpressAdManager alloc] initWithPlacementId:@"900000233" type:PTGNativeExpressAdTypeFeed adSize:CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.height)];
        _manager.delegate = self;
    }
    return _manager;
}
